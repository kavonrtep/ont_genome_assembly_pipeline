...
options_list <- list(
  ...
  make_option(c("-S", "--subject_chromosome_sizes"), action = "store", type = "character",
              help = "file with chromosome sizes for subject(.fai format)", default =
                NULL),
  make_option(c("-o", "--output"), action = "store", type = "character",
              help = "output png file", default = NULL)
)

opt_parser <- OptionParser(option_list = options_list, description = "Creates dotplot
from two blast outputs")
...
tryCatch({
  ...
  chromosome_sizes_query <- read.table(file = opt$query_chromosome_sizes, sep = "\t",
                                                             header = FALSE,
                                       col.names = c('qseqid', 'qlen', 'V3', 'V4', 'V5'))
  chromosome_sizes_subject <- read.table(file = opt$subject_chromosome_sizes, sep =
    "\t", header = FALSE,
                                         col.names = c('sseqid', 'slen', 'V3', 'V4',
                                                       'V5'))

  # --- NEW: Reorder subject contigs by length (largest first) ---
  chromosome_sizes_subject <- chromosome_sizes_subject[order(as.numeric
                                                             (chromosome_sizes_subject$slen), decreasing = TRUE),]
  # Calculate cumulative size for subject
  chromosome_sizes_subject$cumulative_slen <- cumsum(as.numeric
                                                     (chromosome_sizes_subject$slen))
  chromosome_sizes_subject$cumulative_start <- c(1, head
   (chromosome_sizes_subject$cumulative_slen, -1))
  ...
  blast_df$sstart_relative <- blast_df$sstart +
    chromosome_sizes_subject$cumulative_start[match(tolower(blast_df$sseqid),
                                                    tolower(chromosome_sizes_subject$sseqid))]
  blast_df$send_relative <- blast_df$send +
    chromosome_sizes_subject$cumulative_start[match(tolower(blast_df$sseqid),
                                                    tolower(chromosome_sizes_subject$sseqid))]
  ...
  chromosome_sizes_query <- merge(chromosome_sizes_query, median_subject, by =
    "qseqid", all.x = TRUE)
  # Order query contigs by the median subject coordinate (ascending)
  chromosome_sizes_query <- chromosome_sizes_query[order
                                                   (chromosome_sizes_query$subject_mid),]
  # Recalculate cumulative sizes for query based on new order
  chromosome_sizes_query$cumulative_qlen <- cumsum(as.numeric
                                                   (chromosome_sizes_query$qlen))
  chromosome_sizes_query$cumulative_start <- c(1, head
   (chromosome_sizes_query$cumulative_qlen, -1))
  ...
  blast_df$qstart_relative <- blast_df$qstart +
    chromosome_sizes_query$cumulative_start[match(blast_df$qseqid,
                                                  chromosome_sizes_query$qseqid)]
  blast_df$qend_relative <- blast_df$qend +
    chromosome_sizes_query$cumulative_start[match(blast_df$qseqid,
                                                  chromosome_sizes_query$qseqid)]
  ...
  axis(side = 1, at = chromosome_sizes_query$cumulative_qlen -
    chromosome_sizes_query$qlen / 2,
       labels = chromosome_sizes_query$qseqid, tick = FALSE, cex.axis = 0.5, las = 2)
  axis(side = 2, at = chromosome_sizes_subject$cumulative_slen -
    chromosome_sizes_subject$slen / 2,
  ...
  blast_df$qlen <- chromosome_sizes_query$qlen[match(blast_df$qseqid,
                                                     chromosome_sizes_query$qseqid)]
  blast_df$slen <- chromosome_sizes_subject$slen[match(blast_df$sseqid,
                                                       chromosome_sizes_subject$sseqid)]
  blast_df$length <- abs(blast_df$qend - blast_df$qstart)
  blast_df$strand <- ifelse(blast_df$sstrand == "plus", "+", "-")
  blast_df$mapq <- 255

  blast_df$cumulative_qlen <- chromosome_sizes_query$cumulative_qlen[match
                                                                     (blast_df$qseqid,
                                                                      chromosome_sizes_query$qseqid)]
  blast_df$cumulative_slen <- chromosome_sizes_subject$cumulative_slen[match
                                                                       (blast_df$sseqid, chromosome_sizes_subject$sseqid)]
  blast_df$cumulative_start_qlen <- chromosome_sizes_query$cumulative_start[match
                                                                            (blast_df$qseqid, chromosome_sizes_query$qseqid)]
  blast_df$cumulative_strat_slen <- chromosome_sizes_subject$cumulative_start[match
                                                                              (blast_df$sseqid, chromosome_sizes_subject$sseqid)]
  ...
  write.table(PAF, file = opt$output_paf, sep = "\t", quote = FALSE, row.names = FALSE,
              col.names = FALSE)
  ...
  blast_df$qstart_relative_new <- blast_df$cumulative_qlen - blast_df$qstart_relative +
    blast_df$cumulative_start_qlen
  blast_df$qend_relative_new <- blast_df$cumulative_qlen - blast_df$qend_relative +
    blast_df$cumulative_start_qlen
  ...
  # Draw segments for each BLAST hit (using relative coordinates)
  x0 <- ifelse(blast_df$qseqid %in% rc_orientation_id, blast_df$qstart_relative_new,
               blast_df$qstart_relative)
  x1 <- ifelse(blast_df$qseqid %in% rc_orientation_id, blast_df$qend_relative_new,
               blast_df$qend_relative)
  ...
  message(traceback())
  # savinng debugging information to opt$output.RData
  message("Saving debugging information to ", paste0(tools::file_path_sans_ext
                                                     (opt$output), ".RData"))
  save.image(paste0(tools::file_path_sans_ext(opt$output), ".RData"))
}, finally = {
  message("Done.")
})
